...
レスポンスをJSONへ再変換した場合でもAPIが返した `0`、`false`、空文字列を保持できるよう、レスポンスモデルのJSONタグには `omitempty` を付けていません。  
フィールドの有無に意味がある値はポインタで表現します。  
例えば `Item.ElixirCost` は、`nil` がAPIレスポンスに値がないことを表し、値がある場合はポインタの参照先がコストです。
